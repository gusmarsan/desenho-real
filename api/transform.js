...
The drawing is the source of truth. Carefully interpret the child's marks before generating anything. Preserve the main subjects, their relative positions, approximate scale, colors when meaningful, pose/action, scene layout and important small details. Turn simple lines and scribbles into plausible real-world versions of exactly what is present in the drawing.

The final image must always be horizontal and wide. Compose the scene to fill the entire frame edge to edge. If the source drawing is vertical or narrow, expand ONLY the surrounding landscape or continuous background to the left and right. Extend only continuous surfaces, textures and environmental areas that already exist or are clearly implied by the drawing, such as the same sky, ground, grass field, water, wall, floor, horizon or similar background area.

CRITICAL RULE: do not invent or add any new discrete element while widening the scene. Do not add new people, faces, animals, creatures, plants, trees, cars, vehicles, buildings, furniture, objects, props, signs, text, toys, boats, clouds or any other thing that is not already present in the source drawing. Do not multiply existing subjects or objects. The extra horizontal space must contain only a natural continuation of the existing background or landscape. Preserve every original subject as the focus and keep the same number of subjects and meaningful objects as in the drawing.

Do not leave blank side margins, white bars or empty canvas space. Keep the spirit and imagination of a 7-year-old's drawing. Do not over-correct the composition and do not replace the idea with a generic scene. Do not show the paper, frame, marker strokes, crayons or sketch itself in the final result. Make the final image photorealistic, warm, playful, cinematic and safe for children. If a shape is ambiguous, choose the most visually coherent interpretation from the drawing without adding unrelated content.`;
